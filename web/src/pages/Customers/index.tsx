...
export default function Customers() {
  ...
  const loadData = async (page = 1, pageSize = 10) => {
    setLoading(true);
    try {
      const res = await customerApi.list({ page, pageSize });
  ...
  const handleDelete = async (id: number) => { await customerApi.delete(id); message.success('删除成功'); loadData(pagination.current, pagination.pageSize); };
  ...
  return (
    <Card title="客户管理" extra={<Button type="primary" icon={<PlusOutlined />} onClick={handleAdd}>新增客户</Button>}>
